...
class RestaurantServiceTest {
  ...
  //Inject the mock of RestaurantServiceImpl
  @InjectMocks
  ...
  private Double getServingRadius(List<Restaurant> restaurants, LocalTime timeOfService) {
    ...
    return servingRadiusInKms.getValue();
  }

  @Test
  void peakHourServingRadiusOf3KmsAt7Pm() throws IOException {
    assertEquals("3.0", getServingRadius(loadRestaurantsDuringPeakHours(), LocalTime.of(19, 0)).toString());
  }

  @Test
  void peakHourServingRadiusOf3KmsAt2Pm() throws IOException {
    assertEquals("3.0", getServingRadius(loadRestaurantsDuringPeakHours(), LocalTime.of(14, 0)).toString());
  }

  @Test
  void peakHourServingRadiusOf3KmsAt9Pm() throws IOException {
    assertEquals("3.0", getServingRadius(loadRestaurantsDuringPeakHours(), LocalTime.of(21, 0)).toString());
  }

  @Test
  void peakHourServingRadiusOf3KmsAt10Am() throws IOException {
    assertEquals("3.0", getServingRadius(loadRestaurantsDuringPeakHours(), LocalTime.of(10, 0)).toString());
  }
  ...
  void normalHourServingRadiusIs5Kms() throws IOException {
    ...
    // HINT: Use the `loadRestaurantsDuringNormalHours` utility method to speed things up
     ...
     GetRestaurantsResponse response = restaurantService.findAllRestaurantsCloseBy(
      new GetRestaurantsRequest(20.0, 30.0), 
      LocalTime.of(16, 0));

      assertEquals(4, response.getRestaurants().size());
